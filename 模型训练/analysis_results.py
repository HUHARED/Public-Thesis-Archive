...
# 将大体结果保存
listAllClassIdentifyResult = validate_analysis()
temp = []
...
dfBreedsPredicionResults.to_csv("./breed_predicion_results.csv", index=False)
...
for i in range(len(listAllClassIdentifyResult)):
    ...
    if IndexToClass.get(i) is None:
        temp.to_csv("./each_breed_analysis/"+str(i)+".csv", index=False)
    else:
        temp.to_csv("./each_breed_analysis/"+str(IndexToClass.get(i))+".csv", index=False)
