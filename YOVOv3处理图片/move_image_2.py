...
def handle_image(imgPath, movePath):
    ...
    """ 
    有的图片就是报错：AttributeError: 'NoneType' object has no attribute 'shape'
    可能是图片格式不能被opencv正常读取
    那么就直接把这个图片复制走，打印错误信息。
    """
    if image is None:
        print(imgPath)
        ...
        # 有的图根本连猫都不是
        if catCount > 1 or catCount == 0:
            shutil.move(imgPath, movePath)


count = 0
movePath = "./images/wrongimages_two/"
startTime = time.time()
